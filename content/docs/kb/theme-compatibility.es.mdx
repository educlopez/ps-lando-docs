...
`ps-lando` 1.0.0-rc.2 es **theme-agnostic** — suelta cualquier zip con `config/theme.{yml,xml}` y la CLI lo despliega. Pero "la CLI lo despliega" no es lo mismo que "PrestaShop renderiza el front 200 OK". Esta página captura los temas que hemos smoke-tested end-to-end contra PrestaShop 9.1 para que sepas qué esperar antes de invertir tiempo en uno nuevo.
