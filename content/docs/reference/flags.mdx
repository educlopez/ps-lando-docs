...
| Flag | Type | Default | Since | Description |
|---|---|---|---|---|
| `--theme=<name\|path>` | string | _(auto-detect)_ | **1.0** | Theme name (matches a `theme.yml` `<name>` in cwd zips) or `.zip` path. `--theme=none` for explicit no-theme. |
| `--preset=<name>` | string (repeatable) | _(auto-detect)_ | **1.0** | Built-in (`panda`, `none`) or relative path (`./presets/foo.ts`). Repeat to layer. |
| `--no-preset` | bool | `false` | **1.0** | Disable all presets including auto-detected. |
...
| Theme | auto-detect from cwd `*.zip` (or `none` if no theme zip) |
| Preset | `panda` if `panda*.zip` detected, else `none` |
