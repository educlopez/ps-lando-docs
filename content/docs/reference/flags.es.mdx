...
| Flag | Tipo | Default | Desde | Descripción |
|---|---|---|---|---|
| `--theme=<name\|path>` | string | _(auto-detect)_ | **1.0** | Nombre de tema (matchea un `<name>` de `theme.yml` en zips del cwd) o ruta `.zip`. `--theme=none` para sin-tema explícito. |
| `--preset=<name>` | string (repetible) | _(auto-detect)_ | **1.0** | Built-in (`panda`, `none`) o ruta relativa (`./presets/foo.ts`). Repite para apilar. |
| `--no-preset` | bool | `false` | **1.0** | Desactiva todos los presets, incluso los auto-detectados. |
...
| Tema | auto-detect de los `*.zip` del cwd (o `none` si no hay zip de tema) |
| Preset | `panda` si se detecta `panda*.zip`, si no `none` |
