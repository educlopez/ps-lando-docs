...
Desde **1.0.0**, `create` e `install-modules` filtran módulos a través de un único mecanismo genérico: la flag **`--exclude=<glob>`** (repetible, basada en picomatch, case-insensitive). Las flags de grupo pre-1.0 (`--skip-easybuilder`, `--skip-blog`, `--skip-social`, `--skip-marketing`) fueron eliminadas — usarlas ahora termina con exit 64 y un hint de migración en stderr.
...
Cuando ejecutas `ps-lando create` sin `-y` y sin flags `--exclude` / `--only`, el resolver aplica igualmente lo que esté en `pslando.config.json`. Ya no hay un multiselect de módulos — la combinación preset + config dirige el comportamiento. Usa `ps-lando init` para el flujo guiado.
