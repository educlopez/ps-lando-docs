...
Since **1.0.0** (`1.0.0-rc.2`), `ps-lando` is **zero-config and theme-agnostic**: drop your theme + module zips into a folder, run `ps-lando create`, and the CLI walks you through two prompts:

1. **Theme** — `select` prompt listing every theme detected (zip with `config/theme.{yml,xml}` at depth 0 or 1) plus a `None` option.
2. **Modules** — `multiselect` prompt with every module zip detected (`<name>/<name>.php` or `<name>/config.xml`) pre-checked.
...
| `st*.zip` / `module-*.zip` / `<name>.zip` with `<name>/<name>.php` | Any PrestaShop module zip. | Optional — none, one, or many. |
...
| Preset | Activates when | What it does |
|---|---|---|
| `panda` (bundled) | Selected theme name = `panda` | Adds Easy Builder bridge, group filters (`blog`, `easybuilder`, `social`, `marketing`), `stblog_parent`-style ordering. |
