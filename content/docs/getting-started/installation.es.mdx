...
## Suelta cualquier zip de tema

Desde **1.0.0**, `ps-lando` es **zero-config**: suelta cualquier zip de tema de PrestaShop en una carpeta, ejecuta `ps-lando create` y la CLI lo auto-detecta vía `theme.yml` (o el legacy `config.xml`). Ya no necesitas un binario específico para Panda ni la flag `--panda`.

| Patrón | Qué es | ¿Obligatorio? |
|---|---|---|
| `*.zip` con `config/theme.yml` | Cualquier zip de tema de PrestaShop. | Sí — al menos un zip de tema (o `--theme=none`). |
| `st*.zip` / `module-*.zip` | Zips de módulos. | Opcional — se copian/instalan si están presentes. |

La CLI escanea el cwd (o `--zips=<dir>`) y clasifica cada `*.zip` como tema / módulo / desconocido. Los temas se eligen por el `name` de `theme.yml`; los módulos por `<name>/<name>.php` o `<name>/config.xml` en la raíz del zip. Case-insensitive en todo.

### Preset incluido `panda`

Cuando `ps-lando` detecta un `panda*.zip` carga automáticamente el **preset panda** incluido — el mismo comportamiento Panda + Easy Builder que los usuarios pre-1.0 tenían hard-codeado. El preset añade `steasybuilder` + `steasy_trans_panda` como filtros de grupo conocidos y conecta el puente Easy Builder ↔ Panda. Desactívalo con `--no-preset` o `presets: []` en config; fuerza otro con `--preset=<name>`.

| Preset | Se carga cuando | Qué hace |
|---|---|---|
| `panda` (incluido) | `panda*.zip` se detecta en cwd | Añade el puente Easy Builder, filtros de grupo (`blog`, `easybuilder`, `social`, `marketing`), orden tipo `stblog_parent`. |
